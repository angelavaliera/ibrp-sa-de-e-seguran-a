...
export async function getFeaturedArticles(): Promise<BlogArticle[]> {
  try {
    const results = await sanityClient.fetch(
      `*[_type == "article" && publishedAt <= now() && isFeatured == true] | order(publishedAt desc)[0...6] { ${ARTICLE_FIELDS} }`
    );
    if (results && results.length > 0) return results.map(mapArticle);
  } catch (e) {
...
export async function getRecentArticles(excludeSlugs: string[] = []): Promise<BlogArticle[]> {
  try {
    const results = await sanityClient.fetch(
      `*[_type == "article" && publishedAt <= now() && !(slug.current in $excludeSlugs)] | order(publishedAt desc)[0...50] { ${ARTICLE_FIELDS} }`,
      { excludeSlugs }
    );
    if (results && results.length > 0) return results.map(mapArticle);
  } catch (e) {
